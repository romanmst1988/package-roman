...
def test_filter_by_currency_empty_list(empty_transactions_data):
    result = list(filter_by_currency(empty_transactions_data, "USD"))
    assert len(result) == 0

# Тест для transaction_descriptions
def test_transaction_descriptions_correct_descriptions(transactions_data):
...
def test_transaction_descriptions_empty_list(empty_transactions_data):
    result = list(transaction_descriptions(empty_transactions_data))
    assert len(result) == 0

def test_transaction_descriptions_single_transaction(transactions_data):
...
def test_card_number_generator(start, count, expected_first):
...
def test_card_number_generator_zero_range(start, count):
    generator = card_number_generator(start, count)
